---
name: ruflo-status
description: Diagnose K-I Agent health, then report system, MCP server, and active-agent status without changing the installation
argument-hint: "[--fix]"
...
Use this skill when the user asks for K-I Agent health, diagnostics, MCP server
...
authorization for `doctor --fix` from a request to check or diagnose K-I Agent.
