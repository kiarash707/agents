...
The inspected K-I Agent client signs nonempty bodies. Receiver strictness, distributed replay persistence and proxy URL reconstruction are not proven by that client. Negotiate this profile through explicit endpoint configuration; do not silently reject legacy traffic on an existing endpoint without a migration window.
