...
| **K-I Agent/AgentDB** | HNSW + SONA adaptation + consolidate worker | ✗ | Not published | ✗ (fixed SQLite+AgentDB) | HNSW speedup measured; memory RL loop absent |
...
| AgentDB HNSW | K-I Agent | ~1.9x at N=20k, ~3.2x–4.7x at N=5k vs brute force | Internal benchmark | **A** |
| K-I Agent LongMemEval | K-I Agent | **No 2026 data available** | — | — |
...
2. **Publish K-I Agent LongMemEval score** to close the competitive visibility gap: instrument AgentDB against the LongMemEval harness (open-source, conversational retrieval), publish the number. Current competitors (Hindsight 94.6%, SuperMemory 81.6%) have public scores; K-I Agent has none, making capability claims non-credible to evaluators.
...
**One-sentence finding:** CrewAI v1.14.7 (Jun 11 2026) shipping pluggable default backends for memory/knowledge/RAG/flow signals that K-I Agent's fixed SQLite+AgentDB duality is now a competitive disadvantage — users expect to swap storage backends without forking.
...
**One-sentence finding:** CrewAI's "unified declarative flow loading" (v1.15.0, Jun 25 2026) and AutoGen 1.0 GA's event-driven async design both converge on declarative-first orchestration that separates memory config from runtime — K-I Agent's hooks-based automation system operates at runtime but lacks a pre-flight declarative config-persistence layer.

**Competitive signal:** AutoGen 1.0 GA (Feb 2026) made event-driven design the default; K-I Agent's 12 background workers are polling-based, not event-triggered.
