...
**TL;DR:** Budget-Dependent Memory Operator Selection (OAS, 2026) yields +48% task success under tight token budgets and is absent from K-I Agent's `@claude-flow/memory`; implementing it is the highest-ROI memory improvement available in July 2026.
...
|-----------|--------------------|--------------:|:----------:|:--------------:|
| **K-I Agent (current)** | HNSW + SQLite hybrid, SONA, quantized | No | No | No |
...
| K-I Agent HNSW vs brute-force (N=20k) | ~1.9× speedup | A | Internal benchmark, `scripts/benchmark-intelligence.mjs` |
| K-I Agent RaBitQ quantization | 32× compression, 0.60ms/query | A | Internal benchmark, same script |
