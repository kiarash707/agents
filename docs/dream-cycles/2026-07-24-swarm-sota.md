...
| Swarm benchmark suite | None | No ClawArena-equivalent for K-I Agent |
...
| **K-I Agent** | Hierarchical/mesh/adaptive (static) | Raft + optional BFT | None (gap) | 8-agent default | No external benchmark |
...
| K-I Agent HNSW vs brute force (N=20k) | ~1.9x | scripts/benchmark-intelligence.mjs | **A** |
...
**Scan: ruvector-integration** — Qdrant dominates open-source at 10-25% faster p99 than Weaviate or Milvus for <10B vectors; Milvus wins at billion-scale with GPU acceleration. K-I Agent's ruvector NAPI backend has no external connector, capping scale to local SQLite+HNSW.
