# Copilot SDK x K-I Agent Integration — Research & Design
...
**Alternative (external server mode):** run `copilot --headless --port 4321` independently, then connect via `new CopilotClient({ uri: "localhost:4321" })`. This is the preferred mode for K-I Agent because:
...
Additional language packages (not needed for K-I Agent's Node.js surface): `github-copilot-sdk` (Python), `github.com/github/copilot-sdk/go`, `GitHub.Copilot.SDK` (.NET), `github-copilot-sdk` (Rust), `com.github:copilot-sdk-java` (Maven).
...
| MCP server hosting | Indirect | SDK connects TO MCP servers; K-I Agent hosts its own MCP server separately |
...
| Model ID (display name) | SDK model string [unverified-exact] | Provider | Status | Agent | Ask | Edit | CLI | Context Window | Tool Use | Streaming | Multiplier | K-I Agent Tier |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
...
## 5. K-I Agent Today (one-screen recap)

K-I Agent is a **governed software factory** built as a multi-package v3 monorepo under `/Users/cohen/Projects/ruflo/v3/@claude-flow/`.
...
- `client/` — codex has no equivalent because Codex CLI is invoked as a subprocess with no JS SDK. Copilot SDK provides a proper TypeScript client library that must be wrapped for K-I Agent's governance and hook wiring.
...
```json
{
  ...
  "version": "3.8.0",
  "description": "GitHub Copilot SDK integration for K-I Agent (claude-flow) — Copilot platform adapter",
  "type": "module",
  "main": "./dist/index.js",
 ...
 * @claude-flow/copilot
 *
 * GitHub Copilot SDK platform adapter for Claude Flow / K-I Agent
...
export const PACKAGE_INFO = {
  name: '@claude-flow/copilot',
  version: VERSION,
  description: 'GitHub Copilot SDK integration for Claude Flow / K-I Agent',
  platform: 'copilot',
...
**Direction A — Copilot calls INTO K-I Agent MCP (tools available to Copilot agents):**

This is the primary direction. When a Copilot session is created, K-I Agent's MCP server is registered so that Copilot agents can call `memory_store`, `memory_search`, `swarm_init`, `hooks_route`, etc. directly.
...
**Direction B — K-I Agent calls Copilot SDK from within governed workflows:**

This is the `CopilotClient.runGoverned()` path shown in Section 6.5. The SDK spawns the Copilot CLI, sends the compiled prompt, and returns the response. All governance hooks fire. The call is synchronous from K-I Agent's perspective.
...
| 4 | **Client-side vs server-side MCP (ANSWERED)** | Resolved | MCP servers run client-side as local subprocesses or remote HTTP. The Copilot backend does NOT execute MCP tools. K-I Agent's MCP server safely accesses local AgentDB and hooks. [9] |
