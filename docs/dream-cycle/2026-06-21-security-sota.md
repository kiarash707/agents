...
**TL;DR:** Six high-severity arXiv papers published in June 2026 demonstrate that K-I Agent's `ToolOutputGuardrail` covers ASI01 detection but is missing execution-phase sandbox enforcement, cross-interaction memory fragmentation defenses, and task-scoped transactional rollback — three gaps with grade-A benchmark evidence of ≥33% attacker success rates.
...
|-----------|--------------------|--------------------------|-----------------------|--------------------|-------|
| **K-I Agent v3.6** | Detection-only (ToolOutputGuardrail) | None (cross-interaction) | None | ASI01 partial | Integrity verifier |
...
3. **Audit `SafeExecutor` allowlist against CmdNeedle corpus**: Run the CmdNeedle denylist audit (arXiv:2606.15549) against K-I Agent's existing allowlist patterns to confirm there are no bypassable equivalents; add equivalence-class checking (e.g., `sh` ↔ `bash` ↔ `/bin/sh`) to the executor configuration schema.
