...
| mcp-firewall (OSS MCP gateway): default-deny RBAC + Ed25519-signed hash-chained audit trail + fail-closed approval — the closest complete prior-art implementation of what K-I Agent's policy file already declares | github.com/ressl/mcp-firewall (fetched directly) | A |
...
2. **A future security night should scope `dangerousPatterns`/`requireApprovalForDangerous` at the correct layer** — K-I Agent's native-Claude-Code-tool surface (Bash/Write/Edit/WebFetch), not this MCP server, per the policy file's own rationale; that is a materially different, larger piece of work deliberately left out of tonight's patch.
