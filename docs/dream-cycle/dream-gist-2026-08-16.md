...
Ground-truthed both CVEs directly against code. **CVE-2025-59536 (hook RCE):** K-I Agent's own hook dispatch (`.claude/helpers/hook-handler.cjs`, `v3/@claude-flow/hooks/src/registry/`) is a closed set of internal function handlers — no code path executes an arbitrary command string sourced from config. **Not vulnerable through K-I Agent's own dispatch.** **CVE-2025-6514 (OAuth MITM):** no `mcp-remote` dependency anywhere in the repo; K-I Agent's own OAuth client (`v3/@claude-flow/security/src/oauth/client.ts`) targets one hardcoded endpoint (`auth.cognitum.one`) with no dynamic-endpoint-discovery flow to exploit. **Not applicable.**
