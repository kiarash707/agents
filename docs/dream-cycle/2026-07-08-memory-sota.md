...
**TL;DR:** 2026 papers unanimously show passive HNSW retrieval is giving way to RL-navigated multi-granularity memory pyramids; K-I Agent AgentDB remains a passive store and needs an active navigation layer to match the new SOTA.
...
| **K-I Agent AgentDB** | HNSW + sql.js + ONNX 384-dim | Measured 1.9×–4.7× HNSW speedup; no RL nav | ~6K |
...
| K-I Agent AgentDB HNSW (measured) | ~1.9× at N=20k; ~3.2×–4.7× at N=5k (recall@10 ~0.99) | Internal benchmark, ruvector NAPI | A (reproduced) |
...
- **arXiv 2606.20058 (June 2026):** Event-driven async handoffs formalized as first-class orchestration primitives. K-I Agent swarm topology is static post-init.
- **PerspectiveGap benchmark (arXiv 2606.08878):** Same-prompt fan-out produces correlated failures. K-I Agent fan-out lacks prompt-diversity mechanism.
