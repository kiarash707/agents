...
- **Help icon** in sidebar opens "K-I Agent Capabilities" modal (`src/lib/components/RufloHelpModal.svelte`) with all 6 tool groups, quick-start, tips, and resource links.
- **8 K-I Agent-themed example prompts** on the welcome screen (`src/lib/constants/{mcpExamples,routerExamples}.ts`) replacing HF defaults.
- **`PUBLIC_APP_NAME=K-I Agent`** brands the title bar, sidebar, welcome modal, and PWA manifest.
