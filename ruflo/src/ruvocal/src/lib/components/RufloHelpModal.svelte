<script lang="ts">
	...
	const tips = [
		...
		'**AUTO toggle**: green/lit means tool chains continue automatically. Gray means K-I Agent stops after each tool result so you can inspect before continuing.',
	...
	const examplePrompts = [
		...
		'"Search K-I Agent memory for prior decisions about authentication, then web_research recent OAuth2 best practices, in parallel."',
	...
	<div class="flex max-h-[88vh] w-full flex-col overflow-hidden rounded-2xl bg-white text-gray-900 dark:bg-gray-900 dark:text-gray-100">
		<header class="flex items-start justify-between border-b border-gray-200 px-6 py-4 dark:border-gray-700">
			<div class="flex items-start gap-3">
				...
				<div>
					<h2 class="text-lg font-semibold">K-I Agent Capabilities</h2>
		...
		<div class="flex-1 overflow-y-auto px-6 py-4">
			<section class="mb-6">
				...
				<ol class="list-decimal space-y-1.5 pl-5 text-sm">
					...
					<li>K-I Agent decides which tools to call. Watch the streaming tool-call cards below your message — multiple cards = parallel execution.</li>
			...
			<section class="mb-6">
				...
				<div class="mt-3 rounded-lg border border-emerald-300/40 bg-emerald-50/50 px-3 py-2 text-xs dark:border-emerald-700/40 dark:bg-emerald-900/10">
					...
					<p class="mt-1 text-emerald-900/80 dark:text-emerald-200/80">K-I Agent speaks any OpenAI-compatible endpoint: vLLM, Ollama, LM Studio, Together, Groq, or self-hosted. Native support for <a href="https://github.com/ruvnet/RuVector/tree/main/examples/ruvLLM" target="_blank" rel="noopener" class="underline">ruvLLM</a> (lives in <code>ruvnet/RuVector/examples/ruvLLM</code>) — K-I Agent's self-improving local model layer with MicroLoRA adapters that learn from your trajectories via SONA. Pair cloud + local, or run fully offline.</p>
				</div>
			</section>
			...
			<section class="mb-6">
				...
				<p class="mb-2 text-xs text-gray-600 dark:text-gray-400">K-I Agent's web UI is a fork of the open-source HuggingFace chat-ui (SvelteKit) wired to a custom MCP bridge that fans out to two backend kernels. Everything runs on Google Cloud Run.</p>
				<div class="overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700">
					<table class="w-full text-xs">
						<tbody class="divide-y divide-gray-200 dark:divide-gray-700">
							{#each stack as layer}
