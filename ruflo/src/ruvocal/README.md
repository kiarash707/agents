# RuVocal — K-I Agent Web UI

> K-I Agent's multi-model AI chat with built-in Model Context Protocol (MCP) tool calling. Live at [**flo.ruv.io**](https://flo.ruv.io/).
...
RuVocal is the SvelteKit web app that lets you chat with Qwen, Claude, Gemini, or OpenAI while [K-I Agent](https://github.com/ruvnet/ruflo) invokes the same ~210 MCP tools the CLI uses — agent orchestration, persistent memory, swarm coordination, code review, GitHub ops — all directly from chat. No install, no API key needed to try the hosted demo.

It started as a fork of the [HuggingFace chat-ui](https://github.com/huggingface/chat-ui) v0.20.0 and has been extended with a WASM-MCP integration layer, parallel tool execution, an in-browser tool gallery, and a "K-I Agent Capabilities" tour modal. See [ADR-033](../../docs/adr/ADR-033-RUVOCAL-WASM-MCP-INTEGRATION.md) for the architecture.

## What RuVocal adds on top of upstream chat-ui

| | |
|---|---|
| 🛠️ **~210 MCP tools, prefixed** | Five K-I Agent server groups (Core, Intelligence, Agents, Memory, DevTools) plus a 18-tool in-browser WASM gallery |
...
| 📘 **K-I Agent Capabilities modal** | Question-mark icon → multi-section tour: models, tools, architecture, shortcuts |
...
| 🦾 **ruvLLM ready** | Native support for [ruvLLM](https://github.com/ruvnet/RuVector/tree/main/examples/ruvLLM) — K-I Agent's self-improving local model layer |
...
# Curated K-I Agent model list (optional — defaults to /models from the base URL)
TASK_MODEL=qwen/qwen3.6-max-preview
PUBLIC_APP_NAME=K-I Agent
...
## MCP Tools (the K-I Agent difference)
...
```env
MCP_SERVERS=[
  {"name":"K-I Agent Core","url":"https://mcp-bridge-...run.app/mcp/core","transport":"sse"},
  {"name":"K-I Agent Intelligence","url":"https://mcp-bridge-...run.app/mcp/intelligence","transport":"sse"},
  {"name":"K-I Agent Agents","url":"https://mcp-bridge-...run.app/mcp/agents","transport":"sse"},
  {"name":"K-I Agent Memory","url":"https://mcp-bridge-...run.app/mcp/memory","transport":"sse"},
  {"name":"K-I Agent DevTools","url":"https://mcp-bridge-...run.app/mcp/devtools","transport":"sse"},
...
- 🏠 **Parent project** — [K-I Agent](https://github.com/ruvnet/ruflo)
...
MIT — same as the parent [K-I Agent](https://github.com/ruvnet/ruflo) project.
