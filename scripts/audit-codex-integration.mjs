#!/usr/bin/env node
/**
 * Codex ↔ K-I Agent integration audit (issue #1909).
 *
...
console.log('Codex ↔ K-I Agent integration audit (#1909)\n' + '─'.repeat(48));
...
check(/getRufloMcpServerConfig/.test(cfgGen)
  ...
  `default MCP server must use the shared platform-aware K-I Agent definition (cmd /c npx on Windows)`);
