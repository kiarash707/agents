 ...
 * and the K-I Agent value-add. The script appends a category-appropriate
...
// any) and the K-I Agent value-add. Honest about when native is fine — that
...
const SUFFIX = {
  ...
  // -------- Hooks & lifecycle --------
  hooks_: ' Use when native Bash hooks (via Claude Code\'s settings.json) are wrong because you need K-I Agent-side state — pattern persistence, neural training signals, model-routing learning, cost tracking, audit chain. For one-off shell commands, plain Bash hooks are fine.',
  ...
  // -------- Config / system --------
  config_: ' Use when native settings.json edits are wrong because the values need to be read by the K-I Agent runtime (daemon, MCP server, neural router) — those load via the config_* path, not by re-reading settings.json. For .gitignore / .editorconfig style files, native Edit is fine.',
  system_: ' Use when native Bash is wrong because you need K-I Agent runtime metrics (HNSW index size, ReasoningBank state, swarm health, breaker status) — those are not in /proc, only in the running daemon. For OS-level info (uptime, disk, mem), native Bash + standard tools are fine.',
  mcp_: ' Use when native Claude Code MCP status is wrong because you need K-I Agent-side server detail — tool counts per namespace, transport stats, MCP handshake errors. For just "is MCP up?", `claude mcp list` is fine.',
  status_: ' Use when generic K-I Agent health checks are wrong because you want a single quick read of overall system state — daemon up?, swarm initialized?, memory db healthy?, federation peers connected? For deep debugging, prefer the dedicated tools each subsystem exposes.',
  doctor_: ' Use when generic shell debugging is wrong because you want K-I Agent-aware checks — Node/npm versions, daemon, memory DB, API keys, MCP servers, disk space. For unrelated environment troubleshooting, native shell + git/which/env are fine.',
  ...
  security_: ' Use when native package-audit (`npm audit`) is wrong because you need K-I Agent-aware checks — known-bad dep patterns, secret detection, path-traversal in MCP inputs, witness chain verify. For just listing CVEs in your lockfile, native `npm audit` is fine.',
  ...
  // -------- Performance --------
  performance_: ' Use when native shell timing (`time`, `hyperfine`) is wrong because you want K-I Agent-aware benchmarks — HNSW search latency, breaker decisions/sec, MCP response p50/p95, embeddings throughput. For OS-level process profiling, native shell + perf are fine.',
  perf_: ' Use when native shell timing is wrong because you want K-I Agent-aware benchmarks (HNSW, swarm, MCP). For OS-level process profiling, native shell + perf are fine.',
  benchmark_: ' Use when native `time`/`hyperfine` is wrong because you want a K-I Agent-aware suite — agent latency, memory recall accuracy, neural routing hit rate. For OS-level micro-benchmarks, native shell is fine.',
  profile_: ' Use when native Node `--prof` is wrong because you want K-I Agent-component-specific traces (controller-by-controller, hook-by-hook, agent-by-agent). For low-level CPU/heap profiling, native Node profiler + clinic.js are fine.',
  ...
  // -------- Guidance --------
  guidance_: ' Use when generic "what tool should I use?" guessing is wrong — K-I Agent\'s guidance system uses the live tool index + your workflow context to recommend. Pair with hooks_route at task start. For trivial native-only tasks, no guidance call is needed.',

  // -------- IoT (Cognitum Seed) --------
  iot_: ' Use when native ssh-into-device is wrong because you need K-I Agent-tracked fleet state — trust scoring, telemetry anomaly detection, witness chain verification. For one-off device debugging, native ssh is fine.',
  ...
  // -------- Daemon --------
  daemon_: ' Use when native systemd/launchd is wrong because you want to manage just the K-I Agent background workers (12 worker types, priority-aware) without touching OS-level service management. For OS-level service mgmt, native tools are fine.',
...
const CATCHALL = ' Use when native Bash / file tools are wrong because this MCP tool exposes K-I Agent-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.';
