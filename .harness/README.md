...
`harness sign .` produces a witness from `.harness/manifest.json` BUT requires `WITNESS_SIGNING_KEY` (64-char hex Ed25519 private key) in the environment. K-I Agent's existing `brain-signing-key` in GCP Secret Manager is in a different format (89 chars, not hex) and can't be reused.
