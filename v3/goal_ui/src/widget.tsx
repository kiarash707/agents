...
function initRufloResearchWidget(containerId: string = "ruflo-research-widget-container"): void {
  console.log("[K-I Agent Research] Starting initialization...");
  
  const container = document.getElementById(containerId);
  if (!container) {
    console.error(`[K-I Agent Research] Container with id "${containerId}" not found`);
    return;
  }

  console.log("[K-I Agent Research] Container found:", containerId);

  // Apply widget config if provided
  const config = window.RufloResearchWidgetConfig;
  if (config) {
    console.log("[K-I Agent Research] Applying configuration:", config);
  ...
  try {
    ...
    console.log("[K-I Agent Research] ✅ Successfully initialized and rendered");
  } catch (error) {
    console.error("[K-I Agent Research] ❌ Initialization error:", error);
  }
}
...
function autoInit(): void {
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", () => {
      console.log("[K-I Agent Research] DOM ready, auto-initializing...");
      initRufloResearchWidget();
    });
  } else {
    console.log("[K-I Agent Research] DOM already loaded, initializing...");
...
if (typeof window !== "undefined") {
  ...
  console.log("[K-I Agent Research] API exposed on window.RufloResearchWidget");
