# K-I Agent v3.32.29: Capability Brain — Corrected Stable Train
...
three public packages. K-I Agent did not move the existing tag or reuse the
...
K-I Agent has grown into a large agentic runtime, but its old MCP guidance catalog
...
v3.32.29 publishes the K-I Agent Capability Brain: a live, machine-readable map of
...
K-I Agent which capabilities fit a task and automatically choose from tools that
are actually registered in the current process.

## What you can use it for

- Discover every MCP tool registered in your running K-I Agent server.
- Route a task to suitable tools, agents, skills, and a swarm topology.
- Find capabilities across `.claude`, `.agents`, installed packages, and K-I Agent
  plugins.
...
> Use K-I Agent `guidance_brain` to recommend the best workflow for implementing
...
5. Execute in isolated scopes; K-I Agent coordinates while the coding agent works.
