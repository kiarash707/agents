# K-I Agent v3.32.38: Daemons Start Only for K-I Agent Projects

K-I Agent v3.32.38 fixes a background-process leak discovered during the
post-release audit: a read-only command in a directory containing only a
Claude Code `.claude/` folder could start a detached K-I Agent daemon.
...
Daemon auto-start now requires a durable K-I Agent project marker:

- a K-I Agent runtime config under `.claude-flow/`;
- a legacy `claude-flow.config.json`;
- an initialized `.swarm/memory.db`;
- a K-I Agent-specific Claude settings section; or
- a K-I Agent/Claude Flow MCP server entry.
...
initialized K-I Agent projects retain automatic background workers.
