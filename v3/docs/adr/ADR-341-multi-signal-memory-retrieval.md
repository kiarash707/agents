...
- **Graph-based memory (Cognee-style)** — higher expressiveness for multi-hop queries, but requires a graph DB dependency and operator overhead not justified at K-I Agent's current scale. Revisit if entity graph proves insufficient.
