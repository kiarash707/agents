...
1. **AA-AgentPerf** (Artificial Analysis, June 12, 2026): The first agentic inference benchmark has published results showing a 23.6× hardware efficiency gap (GB300: 61,354 Agents/MW vs H200: 2,594 Agents/MW). K-I Agent has no registered performance on this benchmark. All four tracked competitors (LangGraph, AutoGen, CrewAI, OpenAI Swarm) are also unregistered — this is a first-mover opportunity.

2. **Pareto-Optimal Test-Time Scaling** (arXiv 2605.01566, ACL 2026 SRW): Mixture-of-agents (MoA) achieves +2.7pp over self-consistency on MMLU-Pro at equal compute budget. The key configuration insight: MoA is most efficient when `parallelGenerations > sequentialAggregations`. K-I Agent's 3-tier routing (ADR-026) selects by cost/complexity but has no test-time compute scaling mode.
