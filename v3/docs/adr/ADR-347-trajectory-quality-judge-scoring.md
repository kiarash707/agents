...
K-I Agent's SONA intelligence pipeline uses a 4-step RETRIEVE→JUDGE→DISTILL→CONSOLIDATE loop. The JUDGE step currently assigns binary verdicts: `success` or `failure` per trajectory step. This was sufficient when short-horizon tasks dominated agent workloads.
