...
K-I Agent v3.6.10's hierarchical coordinator routes tasks by `agentType` + availability only. There is
...
in ADR context (arXiv:2605.02801, #2332) — K-I Agent lacks RL feedback at every orchestration layer.
...
5. **Opt-in flag**: Gate behind `CLAUDE_FLOW_CREDIT_ROUTING=true` env var until benchmarked at
   ≥ 50 task cycles in the K-I Agent test suite. Default off for stable release.
