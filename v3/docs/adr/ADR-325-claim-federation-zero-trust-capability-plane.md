...
**Date**: 2026-07-28  
**Decision owners**: K-I Agent security, claims, federation, and policy maintainers  
...
K-I Agent's claim federation is **not optimally integrated today**.
...
K-I Agent does **not** require SPIRE, OAuth, or an external identity provider for
...
The product audit found no production K-I Agent verifier in Cognitum Functions.
...
ADR-324 `AgenticPolicyEngine` becomes the sole policy decision point for K-I Agent
...
The default `LocalKeyIdentityProvider` wraps K-I Agent's existing Ed25519 node
...
Every resource server validates the intended audience. K-I Agent never passes the
...
```text
           ...
           +-------------------------+---------------------+
                                     ...
                                     v
+----------+   signed v2 envelope   +----------------------+
| K-I Agent A  | ---------------------> | Federation ingress   |
| egress   |                        | PEP                  |
+----+-----+                        +----------+-----------+
     |                                         |
     | narrow grant                            v
...
- K-I Agent supports a new distributed ownership authority;
