...
- K-I Agent gains a continuous, machine-readable readiness score and MCP threat-model on every PR — the same primitives we use to score third-party repos for harness viability.
...
- **GCP dependency**: `harness validate` uses GCP Secret Manager via `gcloud`. K-I Agent CI must skip those subcommands (or mock them) — explicit `--skip-gcp` flag from the `harness validate` command surface handles this.
...
- K-I Agent baseline scorecard captured: harnessFit 82/100, compileConfidence
...
- K-I Agent genome: repo_type `node_mcp_ci`, risk_score 0.27 (low),
