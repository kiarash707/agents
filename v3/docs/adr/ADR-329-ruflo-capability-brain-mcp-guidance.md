# ADR-329: K-I Agent Capability Brain for MCP Guidance

**Status:** Accepted  
**Date:** 2026-07-28  
**Owners:** K-I Agent CLI, Guidance, Security, and Release Maintainers  
**Related:** ADR-150, ADR-176, ADR-320, ADR-322, ADR-324, ADR-325, ADR-327, ADR-328

## Context

K-I Agent exposes a broad system through MCP tools, CLI commands, agents, skills,
...
This created two failures. Users could not discover most of K-I Agent, and agents
could mistake catalog text or MCP registration for proof that a provider was
configured, reachable, healthy, and authorized.

## Decision

K-I Agent will ship a capability brain behind `guidance_brain` and enhance the
...
| Fact | Meaning |
|---|---|
| `catalogued` | This K-I Agent version describes the capability. |
...
The brain includes K-I Agent's implemented Cognitum identity lifecycle from
...
RFC 8628 polling, a new K-I Agent-specific OAuth client registration, automatic
...
5. **Execute** in isolated scopes; K-I Agent coordinates while an executor works.
...
Users and agents can now ask K-I Agent what it can actually do, receive safe
