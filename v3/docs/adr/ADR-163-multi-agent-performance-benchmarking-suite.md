...
| K-I Agent | **Not published** | Not published | — |

K-I Agent's CLAUDE.md documents performance *targets* (`<100ms MCP`, `<500ms CLI startup`) and internal micro-benchmarks (HNSW speedup, SONA adaptation time), but publishes no end-to-end multi-agent task completion rate, cost-per-task, or throughput-per-dollar figure comparable to what competitors report. This creates a marketing credibility gap and blocks data-driven tuning of the 3-tier routing thresholds.

Two 2026 papers further motivate action:
- **arXiv:2606.19920** (Deep-Unfolded Coordination): distributed task-assignment optimization 6.18–9.44× faster than conventional ADMM solvers — applicable to K-I Agent swarm task decomposition.
- **arXiv:2606.18837** (Skill-MAS): Meta-Skill evolution transfers across unseen tasks and LLMs; K-I Agent's ReasoningBank lacks multi-trajectory rollout.
...
- Reveals whether K-I Agent's ReasoningBank token savings (-32%) translate to fewer retries and higher completion rate.
