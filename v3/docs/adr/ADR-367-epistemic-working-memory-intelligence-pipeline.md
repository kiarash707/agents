...
Two July 2026 papers reveal actionable gaps in K-I Agent's intelligence pipeline:

**SLEUTH (arXiv:2607.12267, Ning Liu, Jul 2026, grade A):** Introduces a structured *epistemic working memory* with three named lists per reasoning task — `confirmedFacts`, `activeHypotheses`, `openQuestions` — maintained across reasoning steps. Result: +11 points on multi-hop benchmarks. K-I Agent's AgentDB stores long-term embeddings but has no per-task structured epistemic buffer.

**GRADE (arXiv:2607.10836, Ghosh & Chakraborty, Jul 2026, grade A):** Hierarchical multi-agent routing with *gated depth control* — learned gates govern agent selection, hierarchy depth, inter-agent communication, and branch pruning via collaborative policy optimization. Result: 44% less inference runtime vs mixture-of-agents baseline, 8.15% accuracy gain. K-I Agent's SONA MoE gates token routing across 8 experts but does not gate agent-selection depth or prune reasoning branches.
