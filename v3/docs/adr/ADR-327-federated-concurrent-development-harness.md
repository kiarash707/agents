...
**Date**: 2026-07-28  
**Decision owners**: K-I Agent Codex, claims, policy, and developer-experience maintainers  
**Related**: ADR-320, ADR-324, ADR-325, ADR-326

## Executive decision

K-I Agent will support repository-local development harnesses as adapters into its
...
but never replace K-I Agent's stronger invariant that every writing agent uses a
distinct Git worktree.

K-I Agent will define one versioned harness contract with:
...
K-I Agent discovers a harness only through tracked repository configuration:
...
Repository scripts remain the preferred local user interface. K-I Agent does not
...
If no tracked harness exists, K-I Agent uses its local claims and policy providers.
...
K-I Agent does not describe them as isolated or receipt-complete. Their dirty
...
K-I Agent coordinates this flow. Codex still performs the implementation and
verification work.

## Compatibility and migration

1. Existing repository harness commands and JSON schemas remain unchanged.
2. K-I Agent first uses an `observe` adapter that imports session, lease, message,
...
6. Older K-I Agent installations ignore the new optional adapter and retain local
   behavior.
...
- K-I Agent can cooperate with existing repository harnesses instead of replacing
  them.
...
harness adds distributed leases, K-I Agent supports a new source-control system, or
