...
Federation (`v3/@claude-flow/plugin-agent-federation/`) is the cross-node trust plane: Ed25519-signed envelopes, peer registry, trust scoring, and a `RoutingService` for sending typed messages between K-I Agent installations. ADR-097 added budget circuit breakers; ADR-086 established the core Ed25519 protocol.

These two modules have been built independently and have **never been wired together**. As a consequence:

- A user running K-I Agent on three nodes — say, a developer laptop, a CI runner, and a long-running cloud worker — cannot have an agent on the laptop hand a claim off to an idle agent on the cloud worker. The work happens locally or not at all.
