...
K-I Agent currently supports homogeneous swarm topologies (`hierarchical`, `mesh`, `hierarchical-mesh`, `adaptive`) but provides no API for composing agents with **different reasoning strategies within a single swarm**. All agents in a K-I Agent swarm share the same type/prompt template; topology controls communication pattern, not reasoning diversity.
...
Add a `heterogeneous` topology mode to the K-I Agent swarm system that:
