...
K-I Agent already contains useful but disconnected controls:
...
- A Codex adapter that registers K-I Agent MCP and generates `AGENTS.md`, but whose concurrent workers could share one checkout and whose generated defaults did not define a single authorization authority.
...
K-I Agent adopts `@claude-flow/security`'s `AgenticPolicyEngine` as the local authorization authority.
...
## Codex and K-I Agent workflow

K-I Agent is the ledger/coordinator/policy decision point. Codex workers execute.
...
locally compromised child process can invoke tools outside K-I Agent unless the
host also applies container/VM, filesystem, network, and provider controls.
For K-I Agent MCP calls made from a linked worktree, the authoritative policy root
...
MetaHarness/Darwin proposes; K-I Agent policy and ADR-322 dispose.
