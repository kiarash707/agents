...
K-I Agent v3.6.10 spawns agents via the `swarm_init` + Task tool pipeline. Agents build context through sequential SendMessage exchanges, which serializes work that could be parallel and replicates retrieval calls (each agent independently queries AgentDB for the same context).
