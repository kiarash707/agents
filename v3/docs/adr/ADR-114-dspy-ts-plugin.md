# ADR-114 — Adopt DSPy.ts as a K-I Agent plugin for declarative prompt-program optimization
...
- **K-I Agent intelligence = run-time, cross-task.** "Across *all* tasks this agent has ever done, what patterns predict success here?" It is always-on, ambient, and not tied to a single signature/metric.
...
**Adopt `dspy.ts` as an optional K-I Agent plugin — `@claude-flow/plugin-dspy` — that wraps the published `dspy.ts` npm package and exposes declarative prompt-program optimization through MCP tools, a slash skill, and an optional agent-loop hook. It shares ruflo's AgentDB instance rather than standing up its own store.**
