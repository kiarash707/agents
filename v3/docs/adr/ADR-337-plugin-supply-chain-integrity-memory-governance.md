...
Two Grade A papers published in April–May 2026 document active supply-chain attacks that bypass all current defenses in K-I Agent's plugin system:
...
K-I Agent's current plugin install path (`v3/@claude-flow/cli/src/plugins/`) fetches from IPFS via Pinata with no signature verification and no semantic intent analysis.
...
**Mnemonic Sovereignty survey** (arXiv:2604.16548, Apr 2026): Identifies nine governance primitives required for secure long-term agent memory. No existing published architecture satisfies all nine. The core gap in K-I Agent: the shared `collaboration` memory namespace (and all AgentDB namespaces) accepts writes from any agent with no per-namespace authorization.
...
- Positions K-I Agent's memory governance ahead of all 2026 competitors (none satisfy all 9 mnemonic sovereignty primitives)
