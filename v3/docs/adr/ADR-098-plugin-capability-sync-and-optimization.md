...
The `plugins/ruflo-*` tree is the user-facing surface of K-I Agent on Claude Code — 32 plugins distributed via the K-I Agent marketplace, each bundling agent prompts, skills, slash commands, and (in some cases) hooks. End users install via `/plugin install ruflo-X@ruflo` and immediately get the agent / commands.
