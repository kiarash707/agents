# ADR-122 — K-I Agent Browser Substrate: signed trajectories, causal self-healing, federated MCTS, session capsules
...
**K-I Agent is not a browser agent. K-I Agent is the memory, policy, replay, and distributed-planning substrate underneath browser agents.**

Stagehand, Browser Use, Surfer-H, Playwright, Chrome, Browserbase, local browsers, and cloud browser pools become **execution targets**. K-I Agent owns: memory, auth state, planning, replay, scoring, policy, learning, and cross-installation coordination.
...
agent → governed Session Capsule → distributed MCTS search →
  ...
  Workflow Compiler → reusable K-I Agent primitive
```
...
1. **Browser Execution Adapters** — Playwright, Stagehand, Browser Use, Browserbase, local Chrome profile, remote browser pool, future Surfer-H visual adapter. Single interface; K-I Agent stays above the tool wars.
