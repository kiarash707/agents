# ADR-318: K-I Agent spinner-verbs surface via Claude Code `spinnerVerbs` settings
...
1. **Append only, never replace.** K-I Agent will never write `mode: "replace"` because that clobbers Claude Code's default verbs (which some users have grown attached to). If a user has already set `replace` themselves, `ruflo spinner enable` refuses with a clear message pointing them to `disable` or manual edit.
...
3. **Namespaced verbs.** K-I Agent tags each verb it appends with an invisible marker (leading zero-width joiner + specific marker sequence) so `disable` can strip only ruflo verbs without touching user-authored ones. See §Removal invariant.
...
**Neutral**
- Users can already do this manually. K-I Agent just makes it opt-in easy + adds a curated, growing pool.
