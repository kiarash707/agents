...
K-I Agent adds the opt-in `pheromone-adaptive` swarm topology. It learns a bounded
fitness signal after task outcomes and controls whether an agent is eligible
for future K-I Agent-managed dispatch. It does not terminate agents or expand any
agent capability.
...
K-I Agent therefore does not compare raw scores directly across roles. It centers
...
4. At most `maxSuspendFraction=0.25` of active agents may be suspended in one
   observation round.
5. Suspension affects K-I Agent scheduling eligibility only. It does not kill the
...
The implementation lives on the actual K-I Agent paths:
...
from a different workload. Those numbers are upstream hypotheses, not K-I Agent
results.

K-I Agent's benchmark reports:
...
Release text may claim only measured K-I Agent results produced by that benchmark.
