...
K-I Agent did **not** wait for a published flywheel. It already ships a mature self-improvement loop in `src/services/`:
...
1. **The PROPOSE step is narrow.** ADR-176 explores by *local deterministic hill-climb over neighbor configs*. `@metaharness/darwin` provides a broader candidate-search mechanism — 7 typed policy surfaces, sandbox scoring, crossover/epistasis, MAP-Elites niches, and a Pareto archive. Its practical superiority over the local proposer must be established on ruflo workloads through controlled quality, cost, latency, and reproducibility comparisons. K-I Agent already shells out to Darwin for **GEPA** (`gepa.mjs`) but **not** as the flywheel's proposer.
2. **The receipt/lineage format is bespoke.** K-I Agent's champions and receipts use their own schema and do not currently round-trip through the `lineage`/`receipts` model exposed by `@metaharness/flywheel`.
3. **`model`/router is not an evolvable surface.** K-I Agent has `@metaharness/router` (hard dep, ADR-321) but the flywheel doesn't evolve routing policy — upstream ADR-145's "8th surface."
4. **No first-class flywheel command surface.** K-I Agent has a mixed surface today: the main CLI exposes `score`, `genome`, `gepa`, `learn`, and related commands, while `evolve` and `bench` are MCP/plugin-only. There is no `flywheel` verb or MCP tool exposing the loop that already exists internally.
...
| Darwin surface | K-I Agent phenotype it mutates |
|---|---|
...
- **Two gates could disagree.** K-I Agent's gate and `@metaharness/flywheel`'s gate may differ; ruflo's is the authority. The package's gate is a CI cross-check, never the promoter.
