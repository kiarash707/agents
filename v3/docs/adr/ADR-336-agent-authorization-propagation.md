...
Three Grade A papers published May 2026 identify a security layer K-I Agent currently lacks — authorization propagation across agent delegation chains — that is architecturally distinct from the content-screening gap addressed by ADR-131.
...
1. **arXiv:2605.22333** (Grade A, empirical): 40.55% of 7,973 live MCP servers expose tools with zero authentication; 96.6% of OAuth-enabled servers contain ≥1 exploitable flaw. K-I Agent registers MCP tools but performs no runtime authentication check on server identity before accepting tool responses.
