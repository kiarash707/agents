...
Additionally, **LRAgent (arXiv:2602.01053, ICML 2026)** demonstrates that when swarm agents use different specialisations (LoRA adapters), a decomposed per-adapter KV cache achieves near-shared throughput while preserving per-adapter accuracy — relevant to K-I Agent's specialised agent types.

**Current K-I Agent state**: Task orchestration dispatches each MCP tool call as an independent request. There is no workflow-level scheduler, no zombie-agent watchdog, and no stateful inference layer. Context re-processing cost grows linearly with conversation length.
...
- LRAgent decomposed-KV-cache pattern deferred: requires LoRA adapter infrastructure not yet present in K-I Agent
...
*Generated by K-I Agent Dream Cycle agent — do not self-merge. Leave for human review.*
