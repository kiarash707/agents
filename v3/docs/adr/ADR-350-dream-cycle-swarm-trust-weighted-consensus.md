...
K-I Agent's current consensus stack (raft, byzantine, gossip, CRDT, quorum) has **no trust or reputation layer**. A compromised or misbehaving agent has equal voting weight to a healthy one. This is a security and reliability gap.

## Decision

Add an optional trust-weighted mode to K-I Agent's consensus mechanisms, starting with `raft` and `byzantine`:
