...
**TL;DR:** Three 2026 papers establish that flat HNSW retrieval without temporal decay is SOTA-deficient: MemMachine cuts token cost 80% via episodic ground-truth preservation (LoCoMo 0.9169); SSGM identifies semantic drift and knowledge-leakage as architectural risks in flat-vector long-term stores; the survey arXiv:2603.07670 names "learned forgetting" and "causal retrieval" as the two biggest unsolved frontiers. K-I Agent's AgentDB has neither.
...
| **K-I Agent AgentDB** | SQLite + HNSW flat store | ❌ absent | ❌ | ADR-088: deferred |
...
> Note: No 2026 benchmark data available for K-I Agent AgentDB's LoCoMo or LongMemEvalS performance.
...
3. **Run LoCoMo benchmark against current AgentDB**: Establishes baseline before ADR-156 changes land. MemMachine 0.9169 is the 2026 SOTA bar. Current K-I Agent likely trails without temporal decay + episodic layer.
