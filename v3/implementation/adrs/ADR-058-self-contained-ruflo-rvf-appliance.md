# ADR-058: Self-Contained K-I Agent RVF Appliance — Linux Kernel + Claude Code + ruvLLM
...
K-I Agent v3.5 requires the user to have Node.js 20+, npm, Claude Code CLI, API keys, and a properly configured OS environment. This means:
...
A **single `ruflo.rvf` file** that contains everything needed to run the full K-I Agent platform:
...
Extending RVF to `RVFA` (RuVector Format Appliance) creates a unified format that K-I Agent already understands natively.
...
The appliance includes a built-in verification suite that tests **every capability** of K-I Agent + Claude Flow. This runs automatically at boot (`Section 5: VERIFY`) and can be triggered manually.
...
```
╔══════════════════════════════════════════════════════════╗
║  K-I Agent Appliance v3.5.2 — Full Capability Verification  ║
...
|---------|----------|-----------------|
| `cloud` | Kernel + Runtime + K-I Agent + Data + Verify | ~60MB |
...
| Cloud profile | Kernel + Runtime + K-I Agent + encrypted keys |
