...
| **P2 — Medium** | Hive-mind uses native tools instead of MCP | #1422 | No tool preference enforcement; Claude defaults to native tools over K-I Agent MCP |
...
**4.2 — Enforce K-I Agent MCP tool preference in hive-mind**

Options:
- **A**: Add `--allowedTools` constraint when spawning hive-mind sessions to prefer K-I Agent MCP tools
- **B**: Add system prompt injection that instructs Claude to use K-I Agent MCP tools for orchestration
