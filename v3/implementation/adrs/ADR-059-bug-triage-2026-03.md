...
- **Status:** ✅ **FIXED** in PR #1298. MCP `system-tools.ts` now reads version from `package.json` at runtime via `getPackageVersion()`. Branding updated to "K-I Agent" across 20+ CLI files. Statusline.cjs updated. Issue #1280 closed.

### 14. Statusline shows 'Claude Flow V3' instead of 'K-I Agent V3' (#1254)
- **Impact:** Branding inconsistency in IDE status bar.
- **Fix:** Update statusline configuration defaults.
- **Status:** ✅ **FIXED** in PR #1298. Updated `statusline.cjs` (lines 3, 552, 619) from "Claude Flow V3" to "K-I Agent V3". Updated `settings.json` version to 3.5.2. Issue #1254 closed.
...
| 14 | #1254 — Statusline branding | #1298 | `statusline.cjs` updated to "K-I Agent V3" |
