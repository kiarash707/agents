...
**Fleet-as-swarm** -- A logistics company manages 500 warehouse sensors as a K-I Agent swarm. Anomaly detection triggers automatic recalibration via agent coordination. No custom dashboard needed -- Claude Code is the interface.

**Edge-cloud federation** -- Edge Seed devices federate with cloud K-I Agent installations using the `@claude-flow/plugin-agent-federation` trust model. Telemetry stays on-premise; only anomaly signatures cross the boundary (PII-gated via the federation plugin).
...
Every Cognitum Seed is modelled as a K-I Agent agent with hardware capabilities. The plugin maintains a `DeviceAgent` entity that wraps a `SeedClient` session and exposes the device's resources (store, OTA, mesh, witness) as agent capabilities. The swarm coordinator manages device agents alongside software agents -- same topology, same trust model, same hooks.

---

## Decision

Build `@claude-flow/plugin-iot-cognitum` as a first-class Claude Flow plugin that bridges Cognitum Seed device fleets into the K-I Agent agent/swarm model with device trust scoring, telemetry-driven anomaly detection, fleet-aware OTA orchestration, and edge-cloud federation.
...
The plugin establishes a bidirectional bridge between Cognitum Seed devices and K-I Agent agents:

```
K-I Agent Agent Swarm                          Cognitum Device Fleet
=================                          =====================
...
The Cognitum SDK's 12 seed endpoints map to K-I Agent agent operations:

| Cognitum SDK | K-I Agent Concept | Plugin Operation |
|-------------|---------------|-----------------|
...
```
Cognitum Seed Device                      K-I Agent Plugin                        AgentDB
====================                      ============                        =======
...
| **Vector store federation** | Device-side HNSW data is queryable from K-I Agent's AgentDB via the `store.query()` bridge -- the device's vector store extends the agent's memory. |
| **Witness chain as audit** | The Seed's Ed25519-signed witness chain provides hardware-rooted provenance that supplements K-I Agent's software audit service. |
...
```typescript
/**
 * A Cognitum Seed device registered with the K-I Agent agent system.
 ...
 * Maps to a K-I Agent swarm with hierarchical topology.
 */
 ...
 * integrates with K-I Agent's audit service.
 */
...
Uses SONA pattern learning from K-I Agent's neural system:
 ...
 * within the K-I Agent plugin context. Handles:
 ...
 * - Health probe coordination with K-I Agent's health check system
 */
...
| **System Under Consideration (SUC)** | The entire device fleet managed by one K-I Agent installation |
...
```
iot_device_register
  Description: Register a new Cognitum Seed device with the K-I Agent agent system
  Input:
...
```typescript
{
  ...
  description: 'Manages fleet topology, policy enforcement, cross-fleet coordination, and fleet-level health assessment. Maps Cognitum mesh topology to K-I Agent swarm topology.',
  defaultConfig: {
    capabilities: [
...
| `iot-mesh-sync` | normal | 2 min | Sync mesh topology from device network layer to K-I Agent swarm view |
...
Device fleets map to K-I Agent swarms with a natural hierarchical topology:

```
Fleet Level (K-I Agent hierarchical swarm)
|
...
- **Fleet Level** uses K-I Agent's swarm coordination (topology, consensus, task assignment).
...
Cross-fleet operations (e.g., "deploy firmware v2.3 to all warehouses") use K-I Agent's hive-mind consensus to coordinate across fleet boundaries.

---

## 12. Integration with Existing K-I Agent Capabilities

### 12.1 Federation Integration (ADR-078)

Edge Seed devices can federate with cloud K-I Agent installations:
  ...
  src/
    ...
    infrastructure/
      cognitum/
        ...
        discovery-adapter.ts           # Adapts SDK discovery to K-I Agent
      persistence/
      ...
      transport/
        mesh-bridge.ts                 # Bridges Cognitum mesh to K-I Agent swarm
...
- Mesh bridge: Cognitum `mesh.status()` / `mesh.peers()` -> K-I Agent swarm topology
...
4. **Witness chain + audit** -- The Seed's Ed25519 witness chain provides hardware-rooted cryptographic provenance. Combined with K-I Agent's software audit service, every operation from "agent decided to update firmware" to "device confirmed new firmware epoch" has a verifiable chain.
...
- Every Cognitum Seed becomes a K-I Agent agent, unifying the physical and software agent mesh.
