...
function concurrentRufloWorkflow(): string {
  return `## K-I Agent + Codex Automated Workflow

K-I Agent is the coordination ledger and policy decision point; Codex workers execute code, tests, and commands. A K-I Agent coordination call records work but never replaces implementation.

Use \`guidance_brain({ mode: "recommend", task: "..." })\` when the task can
benefit from K-I Agent-specific capabilities. Its live registry is authoritative
...
5. **Execute** — Codex workers implement in isolated scopes; K-I Agent records coordination.
