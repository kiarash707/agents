...
export class CodexInitializer {
  ...
  async initialize(options: CodexInitOptions): Promise<CodexInitResult> {
    ...
    try {
      ...
      if (options.skills === undefined) {
        ...
        if (omitted.length > 0) {
          warnings.push(
            ...
            'Install additional capabilities from the K-I Agent plugin catalog.',
          );
        }
      }
      ...
      // gets K-I Agent's lifecycle hooks (PreToolUse/PostToolUse/PreCompact/
  ...
  private async registerMCPServer(): Promise<{ registered: boolean; warning?: string }> {
    try {
      ...
      //
      // Use the shared platform-aware K-I Agent MCP definition so generators,
  ...
  /**
   * #2801 — Install the canonical `ruflo-core@ruflo` plugin so Codex
   * discovers K-I Agent's lifecycle hooks. Idempotent: adds the marketplace
  ...
  private async installRufloCorePlugin(): Promise<{ installed: boolean; warning?: string; activationMessage?: string }> {
    const ACTIVATION = [
      '',
      'ACTION REQUIRED (K-I Agent lifecycle hooks): start a new Codex session, open /hooks,',
      'review the ruflo-core@ruflo hook definitions, and trust them. Use "trust all" only',
      'when every pending definition is from K-I Agent; otherwise trust the K-I Agent definitions',
    ...
    try {
      ...
      return { installed: false, warning: `Could not install K-I Agent plugin. ${MANUAL}` };
    }
  }
...
- K-I Agent: https://github.com/ruvnet/ruflo
`;
  }
...
\`\`\`bash
# Start K-I Agent's MCP server over stdio (dedicated entry point — the
