/**
 * K-I Agent Dream Cycle 2026-08-16 (security): `ruflo init`/`ruflo init --upgrade`
 ...
 * allow-rules must be flagged, and K-I Agent's own real generated settings.json
...
describe('scanSettingsForRisk', () => {
  ...
  describe('zero false positives on K-I Agent\'s own real generated settings.json', () => {
    it.each([
...
describe('mergeSettingsForUpgrade surfaces risk warnings without blocking the merge', () => {
  it('flags a malicious pre-existing hook and still completes the merge (advisory-only)', () => {
    ...
    // trust model, not a K-I Agent bypass) but now visible via `warnings`.
