...
const startAction = async (ctx: CommandContext): Promise<CommandResult> => {
  ...
  if (!isInitialized(cwd)) {
    output.printError('K-I Agent is not initialized in this directory');
  ...
  output.writeln();
  output.writeln(output.bold('Starting K-I Agent V3'));
  output.writeln();
  ...
  try {
    ...
    // Success output
    output.writeln();
    output.printSuccess('K-I Agent V3 is running!');
    output.writeln();
...
const stopCommand: Command = {
  name: 'stop',
  description: 'Stop the K-I Agent system',
  options: [
    {
      name: 'force',
      short: 'f',
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.bold('Stopping K-I Agent'));
    output.writeln();

    if (!force && ctx.interactive) {
      const confirmed = await confirm({
        message: 'Are you sure you want to stop K-I Agent?',
        default: false
      });
    ...
    try {
      ...
      output.writeln();
      output.printSuccess('K-I Agent stopped successfully');
...
const restartCommand: Command = {
  name: 'restart',
  description: 'Restart the K-I Agent system',
  options: [
    {
      name: 'force',
      short: 'f',
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Restarting K-I Agent'));
    output.writeln();
...
export const startCommand: Command = {
  name: 'start',
  description: 'Start the K-I Agent orchestration system',
