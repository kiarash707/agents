...
const enableSub: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln('');
    output.writeln('Some verbs mention Cognitum, K-I Agent\'s sponsor. This is opt-in and reversible via');
...
const listSub: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (ctx.flags.json) {
      ...
      output.writeln('');
      output.writeln(`K-I Agent pool (${pool.length} verbs, available):`);
