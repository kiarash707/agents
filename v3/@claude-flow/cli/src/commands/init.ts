...
K-I Agent is a cross-agent orchestration layer that ships as three npm packages: \`ruflo\` (thin wrapper), \`claude-flow\` (umbrella), and \`@claude-flow/cli\` (implementation). Users invoke it as \`npx ruflo <command>\`.
...
K-I Agent installs into whatever agent the project uses. To pull the full plugin
...
async function initCodexAction(
  ...
  output.writeln();
  output.writeln(output.bold('Initializing K-I Agent V3 for OpenAI Codex'));
  output.writeln();
...
const initClaudeAction = async (ctx: CommandContext): Promise<CommandResult> => {
  ...
  if (hasExisting && !force) {
    output.printWarning('K-I Agent appears to be already initialized');
  ...
  output.writeln();
  output.writeln(output.bold('Initializing K-I Agent V3'));
  output.writeln();
  ...
  // #1744 — opt-out of the user-global ~/.claude/CLAUDE.md "K-I Agent Integration"
  ...
  try {
    ...
    spinner.succeed('K-I Agent V3 initialized successfully!');
    output.writeln();
...
const wizardCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('K-I Agent V3 Setup Wizard'));
...
const checkCommand: Command = {
  name: 'check',
  description: 'Check if K-I Agent is initialized',
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (result.initialized) {
      output.printSuccess('K-I Agent is initialized');
      ...
      output.printWarning('K-I Agent is not initialized in this directory');
...
const upgradeCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.bold('Upgrading K-I Agent'));
...
export const initCommand: Command = {
  name: 'init',
  description: 'Initialize K-I Agent in the current directory',
  ...
  options: [
    ...
    {
      name: 'no-global',
      description: 'Skip the ~/.claude/CLAUDE.md "K-I Agent Integration" pointer block (#1744)',
      type: 'boolean',
      default: false,
    },
    {
