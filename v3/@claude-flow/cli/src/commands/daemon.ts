...
export async function maybeShowMultiDaemonMigrationWarning(opts?: {
  ...
  try {
    ...
    if (anyAiEnabled) {
      output.writeln();
      output.printWarning(`K-I Agent found ${fleet.length} worktree daemons. Scheduled AI workers are now supervisor-gated.`);
      output.printInfo('Inspect:                      ruflo daemon status --all');
      output.printInfo('Stop all:                      ruflo daemon stop --all');
...
async function renderAllDaemonsStatus(): Promise<CommandResult> {
  ...
  if (daemons.length === 0) {
    output.printBox(
      ...
      'K-I Agent Daemons (all workspaces)'
    );
...
`;
    ...
    const unit = `[Unit]
Description=K-I Agent background worker daemon
...
export const daemonCommand: Command = {
  ...
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('K-I Agent Daemon - Background Task Management'));
    output.writeln();
