...
async function checkMcpServers(): Promise<HealthCheck> {
  ...
  if (duplicateLocations.length > 0 || (legacyLocations.length > 0 && currentLocations.length > 0)) {
    ...
    return {
      name: 'MCP Servers',
      status: 'warn',
      message: `Duplicate K-I Agent MCP registrations found (${locations}) — Claude Code will start both tool schemas`,
...
async function checkMcpSchemaOverhead(): Promise<HealthCheck> {
  try {
    ...
    // not K-I Agent command configuration, so there is no corresponding CLI flag.
...
export const doctorCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (fixHandles) {
      output.writeln();
      output.writeln(output.bold('K-I Agent Doctor — fix-handles'));
    ...
    output.writeln();
    output.writeln(output.bold('K-I Agent Doctor'));
