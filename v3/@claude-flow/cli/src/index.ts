 ...
 * Modernized CLI for K-I Agent V3
 *
...
export class CLI {
  ...
  constructor(options: CLIOptions = {}) {
    ...
    this.description = options.description || 'K-I Agent V3 - AI Agent Orchestration Platform';
      ...
      if (commandPath[0] !== 'init' && commandPath[0] !== 'update') {
        ...
        if (commandPath[0] !== 'daemon') {
          try {
            ...
            if (d.started) {
              this.output.printInfo(`Started K-I Agent background daemon for ${process.cwd()} (stop: ruflo daemon stop)`);
            }
          } catch { /* silent */ }
        }
      }
