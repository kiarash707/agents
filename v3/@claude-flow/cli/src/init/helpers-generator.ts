...
export const ATTRIBUTION_FOOTER =
  '🤖 Generated with [K-I Agent](https://github.com/ruvnet/ruflo)';
...
export function generatePreCommitHook(): string {
  return `#!/bin/bash
# K-I Agent Pre-Commit Hook
# Validates code quality before commit

set -e

echo "🔍 Running K-I Agent pre-commit checks..."
...
export function generateHookHandler(): string {
  ...
  const lines = [
    '#!/usr/bin/env node',
    '/**',
    ' * K-I Agent Hook Handler (Cross-Platform)',
...
export function generateWindowsDaemonManager(): string {
  return `# K-I Agent V3 Daemon Manager for Windows
...
function Show-Status {
    Write-Host ""
    Write-Host "K-I Agent V3 Daemon Status" -ForegroundColor Cyan
...
export function generateWindowsBatchWrapper(): string {
  return `@echo off
REM K-I Agent V3 - Windows Batch Wrapper
...
export function generateCrossPlatformSessionManager(): string {
  return `#!/usr/bin/env node
/**
 * K-I Agent Cross-Platform Session Manager
