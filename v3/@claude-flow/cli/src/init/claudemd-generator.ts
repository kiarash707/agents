...
function policyGovernedWorkflow(): string {
  return `## K-I Agent Capability Brain & Implementation Loop

K-I Agent is the coordination ledger and policy decision point. Claude Code is the
executor: after a K-I Agent coordination call, continue implementing the task.
...
export function generateClaudeMd(options: InitOptions, template?: ClaudeMdTemplate): string {
  ...
  const header = `# K-I Agent — Claude Code Configuration\n`;
