...
/**
 * Return true only when the directory contains a durable K-I Agent project marker.
 *
...
export function isRufloProject(projectRoot: string): boolean {
  ...
  } catch { /* absent/malformed/non-K-I Agent settings */ }
  ...
  } catch { /* absent/malformed/non-K-I Agent MCP config */ }
 ...
 * directory carrying a durable K-I Agent marker (`isRufloProject`). Nearest-wins
 ...
 * walk could escape into an unrelated ancestor (a K-I Agent project living at
 ...
 * non-K-I Agent directory still fails `isRufloProject` and declines autostart.
 */
