...
export async function acquireProxyInstallLease(wait: Wait = waitNormally): Promise<() => void> {
  ...
  throw new Error('Another K-I Agent/MetaHarness installer still owns the Meta-Proxy install lease.');
}
...
async function stopEffective(wait: Wait): Promise<EffectiveProxy | null> {
  ...
  if (!isSupportedOwner(owner.executable, process.platform)) {
    throw new Error(`Meta-Proxy port owner pid ${owner.pid} is not a recognized K-I Agent/MetaHarness binary; refusing to signal it.`);
  }
