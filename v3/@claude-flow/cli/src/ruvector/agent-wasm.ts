...
async function attachJsModelProvider(agent: any, config: WasmAgentConfig): Promise<boolean> {
  ...
  const systemPrompt = config.instructions || 'You are a helpful coding assistant running in a K-I Agent WASM agent sandbox.';
...
export async function promptWasmAgent(agentId: string, input: string): Promise<string> {
  ...
  try {
    ...
    const systemPrompt = entry.info.config.instructions || 'You are a helpful coding assistant running in a K-I Agent WASM agent sandbox.';
