 ...
 * Guidance MCP Tools
 *
 * Helps the system navigate K-I Agent's capabilities by providing structured
...
const CAPABILITY_CATALOG: Record<string, CapabilityArea> = {
  'agent-management': {
    ...
    description: 'Spawn, manage, and monitor individual AI agents with lifecycle control. Use when native Bash / file tools are wrong because this MCP tool exposes K-I Agent-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
  ...
  'swarm-orchestration': {
    ...
    description: 'Multi-agent coordination with topology-aware communication and consensus. Use when native Bash / file tools are wrong because this MCP tool exposes K-I Agent-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
  ...
  'memory-knowledge': {
    ...
    description: 'Persistent memory with HNSW vector search, AgentDB storage, and embeddings. Use when native Bash / file tools are wrong because this MCP tool exposes K-I Agent-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
  ...
  'intelligence-learning': {
    ...
    description: 'Neural pattern training (SONA), RL loops, Flash Attention, EWC++ consolidation. Use when native Bash / file tools are wrong because this MCP tool exposes K-I Agent-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
  ...
  'hooks-automation': {
    ...
    description: '17 lifecycle hooks + 12 background workers for automated learning and coordination. Use when native Bash / file tools are wrong because this MCP tool exposes K-I Agent-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
  ...
  'hive-mind': {
    ...
    description: 'Queen-led Byzantine fault-tolerant distributed consensus with multiple strategies. Use when native Bash / file tools are wrong because this MCP tool exposes K-I Agent-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
  ...
  'security': {
    ...
    description: 'Security scanning, CVE remediation, input validation, claims-based authorization. Use when native Bash / file tools are wrong because this MCP tool exposes K-I Agent-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
  ...
  'performance': {
    ...
    description: 'Benchmarking, profiling, metrics collection, and optimization recommendations. Use when native Bash / file tools are wrong because this MCP tool exposes K-I Agent-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
  ...
  'github-integration': {
    ...
    description: 'PR management, code review, issue tracking, release automation, multi-repo coordination. Use when native Bash / file tools are wrong because this MCP tool exposes K-I Agent-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
  ...
  'session-workflow': {
    ...
    description: 'Session state management, workflow execution, task lifecycle, and daemon scheduling. Use when native Bash / file tools are wrong because this MCP tool exposes K-I Agent-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
  ...
  'embeddings-vectors': {
    ...
    description: 'Vector embeddings with sql.js, HNSW indexing, hyperbolic embeddings, ONNX integration. Use when native Bash / file tools are wrong because this MCP tool exposes K-I Agent-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
  ...
  'wasm-agents': {
    ...
    description: 'Sandboxed AI agents running in WebAssembly with virtual filesystem, no OS access. Use when native Bash / file tools are wrong because this MCP tool exposes K-I Agent-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
  ...
  'ruvllm-inference': {
    ...
    description: 'WASM-based HNSW routing, SONA instant adaptation, MicroLoRA, chat formatting. Use when native Bash / file tools are wrong because this MCP tool exposes K-I Agent-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
  ...
  'code-analysis': {
    ...
    description: 'AST analysis, diff classification, coverage routing, dependency graph analysis. Use when native Bash / file tools are wrong because this MCP tool exposes K-I Agent-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
  ...
  'sparc-methodology': {
    ...
    description: 'Specification, Pseudocode, Architecture, Refinement, Completion — structured development. Use when native Bash / file tools are wrong because this MCP tool exposes K-I Agent-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
    tools: [],
    commands: [],
  ...
  'config-system': {
    ...
    description: 'Configuration management, provider setup, system diagnostics, shell completions. Use when native Bash / file tools are wrong because this MCP tool exposes K-I Agent-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
...
const guidanceCapabilities: MCPTool = {
  ...
  description: 'List all capability areas with their tools, commands, agents, and skills. Use this to discover what K-I Agent can do. Use when generic "what tool should I use?" guessing is wrong — K-I Agent\'s guidance system uses the live tool index + your workflow context to recommend. Pair with hooks_route at task start. For trivial native-only tasks, no guidance call is needed.',
  inputSchema: {
    type: 'object',
    properties: {
      area: {
...
const guidanceRecommend: MCPTool = {
  ...
  description: 'Given a task description, recommend which capability areas, tools, agents, and workflow to use. Use when generic "what tool should I use?" guessing is wrong — K-I Agent\'s guidance system uses the live tool index + your workflow context to recommend. Pair with hooks_route at task start. For trivial native-only tasks, no guidance call is needed.',
  inputSchema: {
    type: 'object',
    properties: {
      task: {
...
const guidanceWorkflow: MCPTool = {
  ...
  description: 'Get a recommended workflow template for a task type. Includes steps, agents, and topology. Use when generic "what tool should I use?" guessing is wrong — K-I Agent\'s guidance system uses the live tool index + your workflow context to recommend. Pair with hooks_route at task start. For trivial native-only tasks, no guidance call is needed.',
  inputSchema: {
    type: 'object',
    properties: {
      type: {
...
const guidanceBrain: MCPTool = {
  name: 'guidance_brain',
  description: 'Use when choosing how to execute a task with K-I Agent. Queries the live capability brain, covers every registered MCP tool, separates registration from configuration/reachability/health/authorization, recommends capabilities, and returns the validated implementation loop.',
  inputSchema: {
    type: 'object',
    properties: {
      mode: {
