#!/usr/bin/env node
/**
 * K-I Agent Statusline Generator
...
// #1892 — derive K-I Agent banner version from the installed cli package.json
// so the statusline never drifts from `ruflo doctor`. Falls back to a
// generic "K-I Agent" label only if every resolution path fails.
function resolveBannerVersion() {
  ...
  return ''; // empty → header just says "K-I Agent"
}
...
function generateStatusline() {
  ...
  let header = `${c.bold}${c.brightPurple}▊ K-I Agent${BANNER_VERSION ? ' ' + BANNER_VERSION : ''} ${c.reset}`;
